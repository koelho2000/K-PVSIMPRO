...
export default function App() {
  ...
  // Generate initial hourly load if missing
  useEffect(() => {
  ...
  // UPDATED: Pass behavior to generator
  const updateLoadProfile = (profileId: string) => {
      ...
      if (standard) {
          const hourly = generateSyntheticLoadProfile(
              ...
              standard.baseKw, 
              standard.peakKw,
              standard.behavior // Pass behavior type
          );
  ...
  const handleLoadInputChange = (field: 'annual' | 'base' | 'peak', value: number) => {
      setProject(prev => {
          ...
          if (newProfile.type === 'simplified') {
              // Re-generate with same behavior if possible, else default
  ...
  const handleLoadExportCsv = () => {
     ...
     if (!data || data.length !== 8760) {
        data = generateSyntheticLoadProfile(project.loadProfile.annualConsumptionKwh, project.loadProfile.baseLoadKw, project.loadProfile.peakLoadKw);
     }
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                  ...
                  <input type="number" step="0.01" className="w-full border p-2 rounded" value={project.financialSettings.electricityPriceEurKwh} onChange={(e) => setProject({...project, financialSettings: {...project.financialSettings, electricityPriceEurKwh: parseFloat(e.target.value)}})} />
              </div>
              <div>
                  <label className="block text-sm font-medium mb-1 text-gray-600">Venda de Excedente (€/kWh)</label>
                  <input type="number" step="0.01" className="w-full border p-2 rounded" value={project.financialSettings.gridExportPriceEurKwh} onChange={(e) => setProject({...project, financialSettings: {...project.financialSettings, gridExportPriceEurKwh: parseFloat(e.target.value)}})} />
              </div>
              <div>
                  <label className="block text-sm font-medium mb-1 text-gray-600">Inflação Energética Anual (%)</label>
                  <input type="number" step="0.1" className="w-full border p-2 rounded" value={project.financialSettings.inflationRate} onChange={(e) => setProject({...project, financialSettings: {...project.financialSettings, inflationRate: parseFloat(e.target.value)}})} />
              </div>
          </div>
      </div>
    </div>
    ...
    <div className="space-y-6 h-full pb-10">
        <div className="bg-white p-6 rounded-lg shadow space-y-4 border-l-4 border-blue-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <input className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-200 outline-none" value={project.settings.name} onChange={e => setProject({...project, settings: {...project.settings, name: e.target.value}})} />
                </div>
                <div className="space-y-2">
                    <label className="block text-sm font-medium text-gray-700">Nome Cliente</label>
                    <input className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-200 outline-none" value={project.settings.clientName} onChange={e => setProject({...project, settings: {...project.settings, clientName: e.target.value}})} />
                </div>
            </div>
            <div className="space-y-2 pt-2">
                <label className="block text-sm font-bold text-gray-700">Localização (Concelho)</label>
                <select className="w-full border rounded p-3 bg-gray-50 font-medium text-gray-800 focus:ring-2 focus:ring-blue-200 outline-none" value={project.settings.address} onChange={handleCityChange}>
                    <option value="" disabled>Selecione um concelho...</option>
                    {PORTUGAL_MUNICIPALITIES.map(city => <option key={city.name} value={city.name}>{city.name}</option>)}
                </select>
                <div className="flex justify-between items-center mt-1">
                    <p className="text-xs text-gray-500">Selecionar o concelho gera automaticamente dados climáticos padrão.</p>
                    <div className="flex gap-2">
                        <button onClick={() => epwImportInputRef.current?.click()} className="text-xs text-blue-600 hover:underline flex items-center gap-1"><Upload size={12}/> Importar EPW</button>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
             <div className="mb-4">
                 ...
                 <p className="text-sm text-gray-500">{project.settings.climateDescription}</p>
             </div>
             {project.settings.climateDataSource === 'auto' && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4 rounded-r shadow-sm">
                    <div className="flex items-start">
                        ...
                        <div className="ml-3"><p className="text-sm text-yellow-700"><span className="font-bold">Atenção:</span> Os dados são estimativas sintéticas baseadas na latitude. Recomenda-se EPW.</p></div>
                    </div>
                     <div className="mt-2 ml-8 text-xs text-gray-600">
                         <p className="mb-1 font-bold">Importar Ficheiros EPW:</p>
                         <p>1. Aceda ao <a href="https://www.ladybug.tools/epwmap/" target="_blank" rel="noreferrer" className="text-blue-600 hover:underline flex inline-flex items-center gap-0.5 font-bold">Ladybug Tools EPWMap <ExternalLink size={10}/></a>.</p>
                         <p>2. Clique na localização desejada no mapa.</p>
                         <p>3. Descarregue o ficheiro .epw.</p>
                         <p>4. Carregue-o aqui usando o botão "Importar EPW" acima.</p>
                     </div>
                </div>
             )}
            <div className="bg-orange-50 border border-orange-200 p-4 rounded-lg flex justify-between items-center mb-6">
                 <div><h4 className="font-bold text-orange-900 flex items-center gap-2"><Sun size={20}/> Produtividade Solar (GHI)</h4></div>
                 <div className="text-right"><p className="text-3xl font-extrabold text-orange-600">{totalRad.toLocaleString()}</p><p className="text-sm font-bold text-gray-600">kWh/m²/ano</p></div>
            </div>
  ...
  const renderSystem = () => {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg flex flex-col md:flex-row gap-6 items-center">
          <div className="flex items-center gap-2 text-blue-800 font-bold border-r border-blue-200 pr-6 w-full md:w-auto"><BarChart3 size={24} /><span>Resumo de Consumo <br/><span className="text-xs font-normal">Para Dimensionamento</span></span></div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
              <div className="text-center"><p className="text-xs text-gray-500 uppercase font-bold">Consumo Anual</p><p className="font-bold text-lg">{Math.round(loadStats.total)} kWh</p></div>
              <div className="text-center"><p className="text-xs text-gray-500 uppercase font-bold">Carga Máx (Pico)</p><p className="font-bold text-lg">{loadStats.max.toFixed(2)} kW</p></div>
              <div className="text-center"><p className="text-xs text-gray-500 uppercase font-bold">Média Anual</p><p className="font-bold text-lg">{loadStats.avgYear.toFixed(2)} kW</p></div>
              <div className="text-center bg-white rounded border p-1 shadow-sm"><p className="text-xs text-orange-600 uppercase font-bold">Média Horas Sol</p><p className="font-bold text-lg text-orange-700">{loadStats.avgSun.toFixed(2)} kW</p></div>
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-4 rounded shadow">
           ...
           <select className="w-full border p-2 rounded mb-4" value={project.systemConfig.selectedPanelId} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, selectedPanelId: e.target.value}})}>
           ...
           <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded space-y-2">
              <div className="flex justify-between"><span>Potência:</span> <strong>{panel?.powerW} W</strong></div>
              <div className="flex justify-between"><span>Eficiência:</span> <strong>{(panel?.efficiency || 0) * 100}%</strong></div>
              <div className="grid grid-cols-2 gap-2 text-xs border-t pt-1"><div>Voc: <strong>{panel?.voc} V</strong></div><div>Isc: <strong>{panel?.isc} A</strong></div></div>
              <div className="flex justify-between text-xs"><span>Dim:</span><span className="font-mono">{panel?.widthMm}x{panel?.heightMm}</span></div>
              <div className="flex justify-between text-xs"><span>Peso:</span><strong>{panel?.weightKg} kg</strong></div>
              <div className="mt-2 pt-2 border-t border-gray-200"><p className="text-xs font-bold text-gray-500 uppercase">Potência DC</p><p className="font-bold text-blue-800">{dcPowerKw.toFixed(2)} kWp</p></div>
           </div>
        </div>
        {/* Inverter */}
        <div className="bg-white p-4 rounded shadow">
           <h4 className="font-bold mb-3 flex items-center gap-2"><Zap className="w-4 h-4"/> Inversor</h4>
           <div className="flex gap-2 mb-2">
                <select className="w-full border p-2 rounded" value={project.systemConfig.selectedInverterId} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, selectedInverterId: e.target.value}})}>
                    {INVERTERS_DB.map(i => <option key={i.id} value={i.id}>{i.manufacturer} {i.model} ({i.maxPowerKw}kW)</option>)}
                </select>
                <input type="number" min="1" max="50" className="w-16 border rounded p-2 text-center" value={project.systemConfig.inverterCount || 1} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, inverterCount: Math.max(1, parseInt(e.target.value))}})} />
           </div>
           <button onClick={suggestInverter} className="text-xs text-blue-600 hover:underline mb-4 w-full text-left flex items-center gap-1"><Zap size={12}/> Recomendar Inversor</button>
           <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded space-y-2">
               ...
               <div className="text-xs bg-slate-100 p-2 rounded border mt-2 space-y-1">
                   ...
                   <p className="flex justify-between"><span>Max DC:</span> <strong>{inverter?.maxDcVoltage} V</strong></p>
               </div>
               <div className="mt-2 pt-2 border-t border-gray-200">
                   <p className="text-xs font-bold text-gray-500 uppercase">Potência AC Total</p>
                   <p className="font-bold">{(inverter?.maxPowerKw || 0) * (project.systemConfig.inverterCount || 1)} kW</p>
                   <span className="text-xs text-gray-500">Rácio DC/AC: {ratio.toFixed(2)}</span>
               </div>
           </div>
        </div>
        {/* Battery */}
        <div className="bg-white p-4 rounded shadow">
           ...
           <div className="flex gap-2 mb-4">
               <select className="w-full border p-2 rounded" value={project.systemConfig.selectedBatteryId || ''} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, selectedBatteryId: e.target.value || null}})}>
           ...
           ) : <div className="p-4 border border-dashed rounded text-center text-gray-400 text-sm h-32 flex items-center justify-center">Sem Bateria</div>}
        </div>
      </div>
      <div className="bg-white p-4 rounded shadow">
        <h4 className="font-bold mb-3 flex items-center gap-2"><Settings className="w-4 h-4"/> Cablagem</h4>
        <div className="grid grid-cols-2 gap-6">
            <div>
                <label className="block text-sm font-medium mb-1">Distância DC (m)</label>
                <input type="number" className="border p-2 rounded w-full" value={project.systemConfig.cableDcMeters || 15} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, cableDcMeters: parseFloat(e.target.value)}})} />
            </div>
            <div>
                <label className="block text-sm font-medium mb-1">Distância AC (m)</label>
                <input type="number" className="border p-2 rounded w-full" value={project.systemConfig.cableAcMeters || 10} onChange={(e) => setProject({...project, systemConfig: {...project.systemConfig, cableAcMeters: parseFloat(e.target.value)}})} />
            </div>
        </div>
      </div>
      <div className="bg-blue-50 border border-blue-200 p-6 rounded-lg shadow-inner">
          <div className="flex justify-between items-center mb-6">
              <div><h4 className="text-xl font-bold text-blue-900">Estratégias de Otimização</h4></div>
              <button onClick={handleGenerateScenarios} disabled={isGeneratingScenarios} className="bg-blue-600 text-white px-6 py-3 rounded shadow hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2"><RefreshCw className={isGeneratingScenarios ? "animate-spin" : ""} /> {isGeneratingScenarios ? 'A Gerar...' : 'Gerar 4 Cenários'}</button>
          </div>
          {scenarios.length > 0 && (
             <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                 {scenarios.map(s => (
                     <div key={s.id} className="bg-white p-4 rounded border hover:shadow-lg transition flex flex-col">
                         <div className="border-b pb-2 mb-2"><h5 className="font-bold text-lg text-gray-800">{s.label}</h5><p className="text-xs text-gray-500 h-8">{s.description}</p></div>
                         <div className="space-y-2 text-sm flex-1">
                             <div className="flex justify-between"><span>Potência:</span><span className="font-bold">{s.stats.powerKw} kWp</span></div>
                             <div className="flex justify-between"><span>Painéis:</span><span className="font-bold">{s.stats.panels} un</span></div>
                             <div className="flex justify-between"><span>Inversor:</span><span className="font-bold text-xs">{s.stats.inverterCount}x {s.stats.inverter}</span></div>
                             <div className="flex justify-between"><span>Baterias:</span><span className="font-bold">{s.stats.batteries} un</span></div>
                             <div className="border-t pt-2 mt-2">
                                <div className="flex justify-between text-green-700"><span>Produção:</span><span className="font-bold">{Math.round(s.simulation.totalProductionKwh)} kWh</span></div>
                                <div className="flex justify-between text-blue-700"><span>Autoconsumo:</span><span className="font-bold">{Math.round(s.simulation.totalProductionKwh - s.simulation.totalExportKwh)} kWh</span></div>
                             </div>
                         </div>
                         <button onClick={() => applyScenario(s)} className="mt-4 w-full py-2 bg-gray-800 text-white rounded hover:bg-black text-sm uppercase font-bold tracking-wide">Aplicar</button>
                     </div>
                 ))}
             </div>
          )}
                ...
                <>
                      ...
                      <table className="w-full text-sm text-left border-collapse bg-white shadow rounded-lg overflow-hidden print:shadow-none print:border">
                          <thead className="bg-slate-100 text-slate-600 uppercase text-xs print:bg-slate-200">
                              <tr>
                                  ...
                                  {allProjects.map((p, i) => (
                                      <th key={i} className={`p-4 border-b min-w-[150px] ${i===0 ? 'bg-blue-50 border-blue-200' : ''}`}>
                                          {i===0 ? 'Projeto Atual' : `Cenário ${i}`}
                                          <div className="text-xs font-normal normal-case mt-1 text-gray-500">{p.settings.name}</div>
                                      </th>
                                  ))}
                              </tr>
                          </thead>
                          <tbody className="divide-y">
                              ...
                              <tr>
                                  <td className="p-4 font-bold">Potência (kWp)</td>
                                  {allProjects.map((p, i) => {
                                      const panels = p.roofSegments.reduce((a,b)=>a+b.panelsCount,0);
                                      const panel = PANELS_DB.find(x=>x.id===p.systemConfig.selectedPanelId);
                                      const kwp = (panels * (panel?.powerW||0)) / 1000;
                                      return <td key={i} className="p-4">{kwp.toFixed(2)} kWp</td>
                                  })}
                              </tr>
                              <tr>
                                  <td className="p-4 font-bold">Baterias (kWh)</td>
                                  {allProjects.map((p, i) => {
                                      const bat = BATTERIES_DB.find(b=>b.id===p.systemConfig.selectedBatteryId);
                                      const cap = bat ? bat.capacityKwh * (p.systemConfig.batteryCount||1) : 0;
                                      return <td key={i} className="p-4">{cap.toFixed(1)} kWh</td>
                                  })}
                              </tr>
                              <tr>
                                  <td className="p-4 font-bold">Investimento Estimado</td>
                                  {allProjects.map((p, i) => {
                                      const budget = calculateDetailedBudget(p).reduce((s,x)=>s+x.totalPrice,0) * 1.06;
                              ...
                              <tr>
                                  <td className="p-4 font-bold">Poupança Total</td>
                                  {allProjects.map((p, i) => <td key={i} className="p-4 text-green-700 font-bold">{calculateFinancials(p).totalSavings15YearsEur.toLocaleString('pt-PT', {style:'currency', currency:'EUR', maximumFractionDigits:0})}</td>)}
                              </tr>
                              <tr>
                                  <td className="p-4 font-bold">Payback (Anos)</td>
                                  {allProjects.map((p, i) => <td key={i} className="p-4">{calculateFinancials(p).paybackPeriodYears.toFixed(1)}</td>)}
                              </tr>
                          </tbody>
                      </table>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 print:grid-cols-2 print:gap-4 print:break-inside-avoid">
                      ...
                      <div className="bg-white p-4 rounded shadow border">
                          <h4 className="font-bold text-gray-700 mb-4 text-sm text-center">Balanço Energético (kWh)</h4>
                          <div className="h-64">
                              <ResponsiveContainer width="100%" height="100%">
                                  <BarChart data={chartData}>
                                      ...
                                      <RechartsTooltip />
                                      <Legend />
                                      <Bar dataKey="production" name="Produção" fill="#eab308" radius={[4,4,0,0]} />
                                      <Bar dataKey="load" name="Carga" fill="#94a3b8" radius={[4,4,0,0]} />
                                  </BarChart>
                              </ResponsiveContainer>
                          </div>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded shadow border">
                          <h4 className="font-bold text-gray-700 mb-4 text-sm text-center">Análise Financeira (€)</h4>
                          <div className="h-64">
                              <ResponsiveContainer width="100%" height="100%">
                                  <BarChart data={chartData}>
                                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                      <XAxis dataKey="name" fontSize={12} tickLine={false} axisLine={false} />
                                      <YAxis fontSize={12} tickLine={false} axisLine={false} />
                                      <RechartsTooltip formatter={(val: number) => val.toLocaleString('pt-PT', {style:'currency', currency:'EUR', maximumFractionDigits:0})} />
                                      <Legend />
                                      <Bar dataKey="investment" name="Investimento" fill="#ef4444" radius={[4,4,0,0]} />
                                      <Bar dataKey="savings" name="Poupança 15A" fill="#22c55e" radius={[4,4,0,0]} />
                                  </BarChart>
                              </ResponsiveContainer>
                          </div>
                      </div>
                      ...
                      {/* 3. Efficiency */}
                      <div className="bg-white p-4 rounded shadow border">
                          <h4 className="font-bold text-gray-700 mb-4 text-sm text-center">Eficiência do Sistema (%)</h4>
                          <div className="h-64">
                              <ResponsiveContainer width="100%" height="100%">
                                  <BarChart data={chartData}>
                                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                      ...
                                      <RechartsTooltip />
                                      <Legend />
                                      <Bar dataKey="autoconsumption" name="Autoconsumo" fill="#3b82f6" radius={[4,4,0,0]} />
                                      <Bar dataKey="autonomy" name="Autonomia" fill="#a855f7" radius={[4,4,0,0]} />
                                  </BarChart>
                              </ResponsiveContainer>
                          </div>
                      </div>
                      ...
                      {/* 4. ROI */}
                      <div className="bg-white p-4 rounded shadow border">
                          <h4 className="font-bold text-gray-700 mb-4 text-sm text-center">Retorno do Investimento (ROI %)</h4>
                          <div className="h-64">
                              <ResponsiveContainer width="100%" height="100%">
                                  <BarChart data={chartData}>
                                      <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                      ...
                                      <YAxis fontSize={12} tickLine={false} axisLine={false} unit="%" />
                                      <RechartsTooltip />
                                      <Bar dataKey="roi" name="ROI %" fill="#10b981" radius={[4,4,0,0]} label={{ position: 'top', fill: '#10b981', fontSize: 12 }} />
                                  </BarChart>
                              </ResponsiveContainer>
                          </div>
                      </div>
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
      <aside className="w-64 bg-slate-900 text-slate-100 flex flex-col shadow-xl print:hidden">
        <div className="p-6 border-b border-slate-700 flex flex-col items-center text-center">
          <div className="mb-3"><Logo className="h-16 w-auto text-blue-400" /></div>
        ...
        {/* Footers - Project Actions */}
        <div className="p-4 border-t border-slate-700 bg-slate-950">
            <p className="text-xs font-bold text-slate-500 uppercase mb-2">Projeto</p>
            <div className="flex gap-2">
                <button onClick={exportJson} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white p-2 rounded flex flex-col items-center gap-1 text-xs transition-colors" title="Gravar Projeto JSON">
                    <Download size={18} />
                    <span>Gravar</span>
                </button>
                <button onClick={() => fileInputRef.current?.click()} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white p-2 rounded flex flex-col items-center gap-1 text-xs transition-colors" title="Abrir Projeto JSON">
                    <Upload size={18} />
                    <span>Abrir</span>
                </button>
        ...
        {activeTab === 'load' && (
          <div className="max-w-4xl mx-auto space-y-6">
             <div className="bg-white p-6 rounded shadow border">
                ...
                {/* Import / Export Controls Restored */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="p-4 bg-gray-50 rounded border text-center">
                        <button onClick={() => loadImportInputRef.current?.click()} className="text-sm font-bold text-blue-600 hover:underline flex items-center justify-center gap-2 mx-auto mb-2"><Upload size={16}/> Importar Consumo (8760h)</button>
                        <p className="text-xs text-gray-400">Suporta .txt ou .csv (lista de valores)</p>
                        <input ref={loadImportInputRef} type="file" className="hidden" accept=".csv,.txt" onChange={handle8760Import} />
                    </div>
                    <div className="p-4 bg-gray-50 rounded border text-center">
                        <button onClick={handleLoadExportCsv} className="text-sm font-bold text-green-600 hover:underline flex items-center justify-center gap-2 mx-auto mb-2"><Download size={16}/> Exportar CSV (8760h)</button>
                        <p className="text-xs text-gray-400">Download do perfil atual</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Carga de Pico (kW)</label><input type="number" className="w-full border p-2 rounded" value={project.loadProfile.peakLoadKw} onChange={(e) => handleLoadInputChange('peak', parseFloat(e.target.value))} /></div>
                </div>
             </div>
